...
//class ChatViewController: QMChatViewController , UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate, QMChatServiceDelegate,QMPlaceHolderTextViewPasteDelegate {
//
...
//            // Comment By Sourabh
//            // Retrieving messages
////            if (self.storedMessages()?.count ?? 0 > 0 && self.chatSectionManager.allMessages().count == 0) {
////
...
//
////        (id<QMChatServiceDelegate, QMChatConnectionDelegate>)[[QBMulticastDelegate alloc] init]
//        ServicesManager.instance().chatService.addDelegate(self)
//
//        ServicesManager.instance().chatService.chatAttachmentService.delegate = self
//
//        //        self.chatSectionManager().add(self)
//
//        self.willResignActiveBlock = NotificationCenter.default.addObserver(forName: NSNotification.Name.UIApplication.willResignActiveNotification, object: nil, queue: nil) { [weak self] (notification) in
//
...
//
//        guard let currentMessage = self.chatSectionManager.message(for: indexPath) else {
//            return
//        }
//
...
////            if self.chatSectionManager.me(message) {
////
...
//
//            if let indexPath = self.chatSectionManager.indexPath(for: message) {
...
//    }
//
////    func queueManager() -> QMChatServiceCacheDataSource {
