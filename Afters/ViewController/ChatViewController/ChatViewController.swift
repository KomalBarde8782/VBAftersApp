...
class ChatViewController: QMChatViewController, QMChatServiceDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate {
    ...
    func sendReadStatusForMessage(_ message: QBChatMessage) {
        ...
        if (message.readIDs == nil || !message.readIDs!.contains(currentUserID)) {
            ServicesManager.instance().chatService.read(message, completion: { (error: NSError?) -> Void in
        ...
        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.default).async(execute: {
            ...
            DispatchQueue.main.async(execute: {
                ...
                ServicesManager.instance().chatService.sendAttachmentMessage(message, to: self!.dialog, withAttachmentImage: resizedImage!, completion: {
                    ...
                    self?.attachmentCellsMap.removeObject(forKey: message.id)
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: UInt, senderDisplayName: String!, date: Date!) {
        ...
        message.text = text
        message.senderID = self.senderID
        message.deliveredIDs = [NSNumber(self.senderID)]
        
//        message.deliveredIDs = [(NSNumber(self.senderID))]
        message.readIDs = [NSNumber(self.senderID)]
    ...
    func sendMessage(_ message: QBChatMessage) {
        ...
        // Sending message.
        ServicesManager.instance().chatService.send(message, toDialogID: self.dialog.id!, saveToHistory: true, saveToStorage: true) { (error: NSError?) -> Void in
    ...
    override func collectionView(_ collectionView: QMChatCollectionView!, minWidthAt indexPath: IndexPath!) -> CGFloat {
        ...
        if self.dialog.type != QBChatDialogType.private {
            ...
            let topLabelSize = TTTAttributedLabel.sizeThatFitsAttributedString(self.topLabelAttributedStringForItem(item), withConstraints: CGSize(width: (collectionView.frame).width - kMessageContainerWidthPadding, height: CGFloat.max), limitedToNumberOfLines:0)
    ...
    override func collectionView(_ collectionView: QMChatCollectionView!, layoutModelAt indexPath: IndexPath!) -> QMChatCellLayoutModel {
        ...
        if viewClass === QMChatIncomingCell.self || viewClass === QMChatAttachmentIncomingCell.self {
            ...
            if self.dialog.type != QBChatDialogType.private {
                ...
                let size = TTTAttributedLabel.sizeThatFitsAttributedString(topAttributedString, withConstraints: CGSize(width: (collectionView.frame).width - kMessageContainerWidthPadding, height: CGFloat.max), limitedToNumberOfLines:1)
        ...
        if self.detailedCells.contains(item.id!) {
            ...
            size = TTTAttributedLabel.sizeThatFitsAttributedString(bottomAttributedString, withConstraints: CGSize(width: (collectionView.frame).width - kMessageContainerWidthPadding, height: CGFloat.max), limitedToNumberOfLines:0)
        }
        ...
        if let attachmentCell = cell as? QMChatAttachmentCell {
            ...
            if let attachment = message?.attachments?.first {
                ...
                ServicesManager.instance().chatService.chatAttachmentService.getImageForAttachmentMessage(message!, completion: {
                    ...
                    self?.attachmentCellsMap.removeObject(forKey: attachment.id)
