...
//                        SVProgressHUD.showError(response.error?.errorwithStatus,,: ?.localizedDescription)
                        return
                    }
                ...
                let usersInDialogs = dialogOccupants
                
//                commented by Suyog
//                let usersInDialogs = usersWithoutCurrentUser.filter({dialogOccupants.contains(NSNumber(($0 as QBUUser).id))})
...
//                    var chatName = text!.stringByTrimmingCharactersInSet(CharacterSet.whitespaceCharacterSet())
                    var chatName = text!.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
                    if chatName.isEmpty {
    ...
    func updateDialog(_ dialog:QBChatDialog, newUsers users:[QBUUser], completion: ((_ response: QBResponse?, _ dialog: QBChatDialog?) -> Void)?) {
        ...
        ServicesManager.instance().chatService.joinOccupants(withIDs: usersIDs, to: dialog) { [weak self] (response: QBResponse, dialog: QBChatDialog?) -> Void in
            ...
            //Commented Suyog
//            ServicesManager.instance().chatService.sendSystemMessageAboutAdding(to: unwrappedDialog, toUsersIDs: usersIDs, withText:notificationText, completion: { (error: NSError?) -> Void in
//                
